...
resource "google_cloud_run_v2_service_iam_member" "hello-app-target_run_developer" {
  ...
  member   = "serviceAccount:${google_service_account.hello-app-target[each.value.env].email}"
}

resource "google_clouddeploy_target" "regional-hello-app" {
  for_each = local.targets

  project          = google_project.pipeline.project_id
  location         = var.region0
  name             = "hello-app-${each.value.env}-${each.value.region}"
  require_approval = false

  execution_configs {
    usages           = ["RENDER", "DEPLOY"]
    service_account  = google_service_account.hello-app-target[each.value.env].email
    artifact_storage = "gs://${google_storage_bucket.storage.name}/artifacts"
  }

  run {
    location = "projects/${each.value.project}/locations/${each.value.region}"
  }

  deploy_parameters = {
    message              = "Hello, ${each.value.env} from ${each.value.region}!"
    service_account_name = google_service_account.hello-app[each.value.env].email
  }
...
resource "google_clouddeploy_target" "hello-app" {
  ...
  multi_target {
    target_ids = [ for i, _ in local.regions : google_clouddeploy_target.regional-hello-app["${each.key}-${i}"].id ]
  }
}
